...
CProposalVote::CProposalVote(const CPubKey& votingKeyIn, const uint256& nProposalHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn)
    : fValid(true),
      fSynced(false),
      nVoteSignal(eVoteSignalIn),
      votingKey(votingKeyIn),
...
bool CProposalVote::Sign(const CKey& pkVotingKey)
{
    std::string strError;

    uint256 hash = GetSignatureHash();

    if(!CHashSigner::SignHash(hash, pkVotingKey, vchSig)) {
        LogPrintf("CProposalVote::Sign -- SignHash() failed\n");
        return false;
    }

    if (!CHashSigner::VerifyHash(hash, votingKey, vchSig, strError)) {
...
bool CProposalVote::CheckSignature() const
{
    ...
    if (!CHashSigner::VerifyHash(hash, votingKey, vchSig, strError)) {
        ...
        std::string strMessage = votingKey.GetID().ToString() + "|" + nProposalHash.ToString() + "|" +
            boost::lexical_cast<std::string>(nVoteSignal) + "|" +
            boost::lexical_cast<std::string>(nVoteOutcome) + "|" +
            boost::lexical_cast<std::string>(nTime);

        if(!CMessageSigner::VerifyMessage(votingKey, vchSig, strMessage, strError)) {
...
bool CProposalVote::IsValid(bool fSignatureCheck) const
{
    if(nTime > GetAdjustedTime() + (60*60)) {
        LogPrint("proposal", "CProposalVote::IsValid -- vote is too far ahead of current time - %s - nTime %lli - Max Time %lli\n", GetHash().ToString(), nTime, GetAdjustedTime() + (60*60));
        return false;
    }
    ...
    {
        LogPrint("proposal", "CProposalVote::IsValid -- Client attempted to vote on invalid signal(%d) - %s\n", nVoteSignal, GetHash().ToString());
        return false;
    }
    ...
    if(nVoteOutcome > 3)
    {
        LogPrint("proposal", "CProposalVote::IsValid -- Client attempted to vote on invalid outcome(%d) - %s\n", nVoteSignal, GetHash().ToString());
        return false;
    }
...
{
    bool fResult = ((vote1.votingKey == vote2.votingKey) &&
...
bool operator<(const CProposalVote& vote1, const CProposalVote& vote2)
{
    ...
    fResult = (vote1.votingKey == vote2.votingKey);
