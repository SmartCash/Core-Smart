...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char *pszDest, bool fConnectToSmartnode)
{
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        std::set<std::vector<unsigned char> > setConnected;
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fSmartnode) {
    ...
    while (!interruptNet)
    {
        ...
        while(it != p.second.end()) {
            if(*it != uint256()) {
                ...
                LogPrint("smartnode", "ThreadMnbRequestConnections -- asking for mnb %s from addr=%s\n", it->ToString(), p.first.ToString());
            }
            ++it;
        }
...
void CConnman::RelayTransaction(const CTransaction& tx)
{
    ...
    //     ss << dstx;
    // } else 
    if(instantsend.GetTxLockRequest(hash, txLockRequest)) { // MSG_TXLOCK_REQUEST
        ss << txLockRequest;
    } else { // MSG_TX
        ss << tx;
    }
