...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SmartCash users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "SmartCash Core " + FormatFullVersion();
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    if ((addrman.size() > 0) &&
        ...
        for (auto pnode : vNodes) {
            nRelevant += pnode->fSuccessfullyConnected && !pnode->fFeeler && !pnode->fOneShot && !pnode->fInbound;
        }
        if (nRelevant >= 2) {
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(host.c_str(), vIPs, 256, true))
            {
                BOOST_FOREACH(const CNetAddr& ip, vIPs)
                {
                    ...
                    vAdd.push_back(addr);
                    LogPrintf(" => %s found from %s\n", ip.ToString(), seed.host);
                    found++;
                }
...
void CConnman::ThreadOpenConnections()
{
    ...
    while (!interruptNet)
    {
        ...
        if (!Params().AllowMultipleAddressesFromGroup()) {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes) {
                if (!pnode->fInbound && !pnode->fAddnode && !pnode->fSmartnode) {
        ...
        while (!interruptNet)
        {
            ...
            if (MainNet() && addr.GetPort() != Params().GetDefaultPort() && nTries < 50)
                continue;
...
void CConnman::ThreadOpenAddedConnections()
{
    ...
    while (true)
    {
        CSemaphoreGrant grant(*semAddnode, true);
...
void CConnman::ThreadOpenSmartnodeConnections()
{
    ...
    if (mapArgs.count("-connect") && mapMultiArgs.at("-connect").size() > 0)
        return;
    ...
    while (!interruptNet)
    {
        ...
        CSemaphoreGrant grant(*semSmartnodeOutbound, true);
        ...
        const CService addr = vPendingSmartnodes.front();
        vPendingSmartnodes.erase(vPendingSmartnodes.begin());
        if (IsSmartnodeOrDisconnectRequested(addr)) {
...
bool CConnman::OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound, const char *pszDest, bool fOneShot, bool fFeeler, bool fAddnode, bool fConnectToSmartnode)
{
    //
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SmartCash Core is probably already running."), addrBind.ToString());
        else
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (semSmartnodeOutbound == NULL) {
        // initialize semaphore
        semSmartnodeOutbound = new CSemaphore(MAX_OUTBOUND_SMARTNODE_CONNECTIONS);
    }
    ...
    threadOpenSmartnodeConnections = std::thread(&TraceThread<std::function<void()> >, "mnbcon", std::function<void()>(std::bind(&CConnman::ThreadOpenSmartnodeConnections, this)));
...
void CConnman::Interrupt()
{
    ...
    if (semSmartnodeOutbound) {
        for (int i=0; i<MAX_OUTBOUND_SMARTNODE_CONNECTIONS; i++) {
