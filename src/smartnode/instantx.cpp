...
// step 2) Top COutPointLock::SIGNATURES_TOTAL smartnodes per each spent outpoint push "txvote" message
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate, CConnman& connman)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int nRank;
        if(!mnodeman.GetSmartnodeRank(activeSmartnode.outpoint, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
            LogPrint("instantsend", "CInstantSend::Vote -- Can't calculate rank for smartnode %s\n", activeSmartnode.outpoint.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(nRank > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Smartnode not in the top %d (%d)\n", nSignaturesTotal, nRank);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasSmartnodeVoted(itOutpointLock->first, activeSmartnode.outpoint)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeSmartnode.outpoint);
...
bool CInstantSend::ResolveConflicts(const CTxLockCandidate& txLockCandidate)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, txLockCandidate.txLockRequest.vin) {
        ...
        if(GetLockedOutPointTxHash(txin.prevout, hashConflicting) && txHash != hashConflicting) {
            ...
            //       (not a big issue since we already PoSe ban malicious smartnodes
            //        and they won't be able to spam)
            // TODO: ban all malicious smartnodes permanently, do not accept anything from them, ever
...
int64_t CInstantSend::GetAverageSmartnodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapSmartnodeOrphanVotes is empty
    if(mapSmartnodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapSmartnodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapSmartnodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(itOrphanVote->second.IsTimedOut()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan vote: txid=%s  smartnode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetSmartnodeOutpoint().ToStringShort());
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsFailed()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing vote for failed lock attempt: txid=%s  smartnode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetSmartnodeOutpoint().ToStringShort());
            mapTxLockVotes.erase(itVote++);
        } else {
            ++itVote;
    ...
    // remove timed out smartnode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itSmartnodeOrphan = mapSmartnodeOrphanVotes.begin();
    while(itSmartnodeOrphan != mapSmartnodeOrphanVotes.end()) {
        if(itSmartnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing timed out orphan smartnode vote: smartnode=%s\n",
                    itSmartnodeOrphan->first.ToStringShort());
            mapSmartnodeOrphanVotes.erase(itSmartnodeOrphan++);
        } else {
            ++itSmartnodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(outpointSmartnode)) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown smartnode %s\n", outpointSmartnode.ToStringShort());
        mnodeman.AskForMN(pnode, outpointSmartnode, connman);
        return false;
    }
    ...
    int nRank;
    if(!mnodeman.GetSmartnodeRank(outpointSmartnode, nRank, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION)) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Can't calculate rank for smartnode %s\n", outpointSmartnode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Smartnode %s, rank=%d\n", outpointSmartnode.ToStringShort(), nRank);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(nRank > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Smartnode %s is not in the top %d (%d), vote hash=%s\n",
                outpointSmartnode.ToStringShort(), nSignaturesTotal, nRank, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    if(!mnodeman.GetSmartnodeInfo(outpointSmartnode, infoMn)) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Smartnode: smartnode=%s\n", outpointSmartnode.ToString());
        return false;
    }

    if(!CMessageSigner::VerifyMessage(infoMn.pubKeySmartnode, vchSmartnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!CMessageSigner::SignMessage(strMessage, vchSmartnodeSignature, activeSmartnode.keySmartnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!CMessageSigner::VerifyMessage(activeSmartnode.pubKeySmartnode, vchSmartnodeSignature, strMessage, strError)) {
...
{
    if(mapSmartnodeVotes.count(vote.GetSmartnodeOutpoint()))
        return false;
    mapSmartnodeVotes.insert(std::make_pair(vote.GetSmartnodeOutpoint(), vote));
    return true;
}
...
bool COutPointLock::HasSmartnodeVoted(const COutPoint& outpointSmartnodeIn) const
{
...
bool CTxLockCandidate::HasSmartnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointSmartnodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasSmartnodeVoted(outpointSmartnodeIn);
}
