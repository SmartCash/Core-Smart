...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse smartcash.conf
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("Smartcash Core"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    // smartcash: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Smartcash Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
