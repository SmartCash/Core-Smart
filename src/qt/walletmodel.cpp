...

    {
        ...
        if(recipients[0].fUseInstantSend) {
            if(newTx->GetValueOut() > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                Q_EMIT message(tr("Send Coins"), tr("InstantPay doesn't support sending values that high yet. Transaction amount and fee is currently limited to %1 SMART.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
                             CClientUIInterface::MSG_ERROR);
                return TransactionCreationFailed;
            }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal smartcash:URI (smartcash:S123...?message=example)
...
{
    std::vector<COutput> vCoins;
    wallet->AvailableCoins(vCoins, false);// false adds unconfirmed/timelocked for coincontrol list
