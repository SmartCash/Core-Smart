        ...
        if ((nHeight > 0) && (nHeight <= 717499999)) {
            ...
            if ((nHeight > 0) && (nHeight < 90000)) {
                BOOST_FOREACH(const CTxOut &output, tx.vout) {
                    ...
                    if (output.scriptPubKey == FOUNDER_2_SCRIPT && abs(output.nValue - (int64_t)(0.08 * (GetBlockValue(pindexBestHeader->nHeight+1, 0, pindexBestHeader->nTime)))) < 2 ) {
                        found_2 = true;
                    }
                    if (output.scriptPubKey == FOUNDER_3_SCRIPT && abs(output.nValue - (int64_t)(0.08 * (GetBlockValue(pindexBestHeader->nHeight+1, 0, pindexBestHeader->nTime)))) < 2 ) {
                        found_3 = true;
                    }
                    if (output.scriptPubKey == FOUNDER_4_SCRIPT && abs(output.nValue - (int64_t)(0.15 * (GetBlockValue(pindexBestHeader->nHeight+1, 0, pindexBestHeader->nTime)))) < 2 ) {
                        found_4 = true;
                    }
                    if (output.scriptPubKey == FOUNDER_5_SCRIPT && abs(output.nValue - (int64_t)(0.56 * (GetBlockValue(pindexBestHeader->nHeight+1, 0, pindexBestHeader->nTime)))) < 2 ) {
                        found_5 = true;
                    }
                }
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
	...
	int nHeight = getNHeight(block);
    if (fCheckPOW && !CheckProofOfWork(nHeight, block.GetHash(), block.nBits, consensusParams))
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, CNode* pfrom, const CBlock* pblock, bool fForceProcessing, const CDiskBlockPos* dbp, bool fMayBanPeerIfInvalid)
{
    ...
    smartnodeSync.IsBlockchainSynced(true);
...
//                LogPrintf("inv.type()=%s, inv.GetCommand=%s\n", inv.type, inv.GetCommand());
                ...
                if (!pushed && inv.type == MSG_SMARTNODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapSmartnodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::SMARTNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }
                        ...
                        BOOST_FOREACH(CSmartnodePayee& payee, mnpayments.mapSmartnodeBlocks[mi->second->nHeight].vecPayees) {
                            ...
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapSmartnodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::SMARTNODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_SMARTNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenSmartnodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSmartnodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_SMARTNODE_PING) {
                    if(mnodeman.mapSeenSmartnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSmartnodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_SMARTNODE_VERIFY) {
                    if(mnodeman.mapSeenSmartnodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenSmartnodeVerification[inv.hash];
