...
{
    size_t nRelayNodes = fReachable ? 2 : 1; // limited relaying of addresses outside our network(s)
    ...
    auto sortfunc = [&best, &hasher, nRelayNodes](CNode* pnode) {
        if (pnode->nVersion >= CADDR_TIME_VERSION) {
            ...
            for (size_t i = 0; i < nRelayNodes; i++) {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_SMARTNODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        ...
                        connman.PushMessage(pfrom, NetMsgType::SMARTNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION)
        {
            ...
            LogPrint("net", "peer=%d using obsolete version %i; disconnecting\n", pfrom->id, nVersion);
        ...
        if(!smartnodeSync.IsSynced() && !pfrom->fDisconnect) {
            ...
            LogPrintf("CNode::CNode -- requesting sporks from peer %d\n", pfrom->id);
        }
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        //         LogPrint("privatesend", "DSTX -- Can't find smartnode %s to verify %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
        //         return false;
        //     }

        //     if(!mn.fAllowMixingTx) {
        //         LogPrint("privatesend", "DSTX -- Smartnode %s is sending too many transactions %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
        //         return true;
        //         // TODO: Not an error? Could it be that someone is relaying old DSTXes
        //         // we have no idea about (e.g we were offline)? How to handle them?
        //     }

        //     if(!dstx.CheckSignature(mn.pubKeySmartnode)) {
        //         LogPrint("privatesend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
        //         return false;
        //     }

        //     LogPrintf("DSTX -- Got Smartnode transaction %s\n", hashTx.ToString());
...
bool SendMessages(CNode* pto, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    const Consensus::Params& consensusParams = chainParams.GetConsensus();
    {
        ...
        vector<CInv> vInvWait;
        {
            ...
            LOCK(pto->cs_inventory);
            vInv.reserve(std::min<size_t>(MAX_INV_SZ, pto->vInventoryToSend.size()));
